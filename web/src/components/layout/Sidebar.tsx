...
import { activeView, setActiveView } from '../../stores/view';
...
export default function Sidebar(props: SidebarProps) {
  ...
  return (
    ...
    >
      ...
      {/* View switcher */}
      <nav class="flex gap-0.5 px-2 py-2 border-b border-border">
        <button
          class={`flex items-center gap-2 px-2 py-1.5 text-sm rounded-md transition-colors flex-1 ${
            activeView() === 'agents'
      ...
      {/* Agents view — agent list + sessions */}
                ...
                {(agent) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        selectAgent(ns, name);
                        // Always show agent detail view when clicking an agent
                    ...
                    >
                      ...
                      <Show when={agent.mode}>
                        <span class="text-xs text-text-muted">{agent.mode}</span>
                      </Show>
                    </button>
                  );
                }}
      ...
      {/* Runs view — placeholder, run list is in the main content area */}
      <Show when={activeView() === 'runs'}>
        <div class="flex-1 flex items-center justify-center px-4">
          <p class="text-xs text-text-muted text-center">
            Agent runs are shown in the main area.
          </p>
        </div>
      </Show>
